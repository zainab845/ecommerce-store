...
export default function CartPage() {
  ...
  const premiumDiscount = isPremium ? totalPrice * 0.1 : 0;
    ...
    try {
      const res = await fetch('/api/coupons/validate', {
        ...
        body: JSON.stringify({
          ...
          orderAmount: totalPrice, // validate against pre-premium price
        }),
      });
      const data = await res.json();
      if (res.ok) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-5 flex items-center gap-3 px-4 py-3 bg-emerald-50 border border-emerald-100 rounded-2xl">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-600">
              You save ${premiumDiscount.toFixed(2)} (10% off) on this order
            </p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4 mb-8">
        {items.map(item => (
          <div key={item.id} className="flex gap-3 sm:gap-4 p-4 bg-white border border-gray-100 rounded-2xl">
            <img
      ...
      <div className="bg-gray-50 rounded-2xl p-5 sm:p-6">
        ...
        <div className="mb-4">
          {appliedCoupon ? (
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-indigo-400 transition-colors uppercase"
              />
              <button
                onClick={handleApplyCoupon}
                disabled={!couponCode.trim() || couponLoading}
                className="px-4 py-2.5 bg-indigo-600 text-white text-sm font-semibold rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-colors flex-shrink-0"
              >
                {couponLoading ? '...' : 'Apply'}
              </button>
            </div>
          ...
          {couponSuccess && !appliedCoupon && <p className="mt-1.5 text-xs text-green-600 font-medium">{couponSuccess}</p>}
        </div>
